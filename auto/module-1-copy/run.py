...
PRODUCT_TEMPLATE_PATH = SCRIPT_DIR / "product.template.md"
...
def main() -> int:
    try:
        ...
        if not PRODUCT_TEMPLATE_PATH.is_file():
            raise RunError(f"Không tìm thấy file template: {PRODUCT_TEMPLATE_PATH}")

        try:
            template = PRODUCT_TEMPLATE_PATH.read_text(encoding="utf-8-sig")
        except OSError as exc:
            raise RunError(
                f"Không thể đọc file {PRODUCT_TEMPLATE_PATH}: {exc}"
            ) from exc

        # Luôn khôi phục toàn bộ product.md từ template trước mỗi lần render.
        atomic_write_text(PRODUCT_MARKDOWN_PATH, template)
        print(
            f"Đã ghi đè {PRODUCT_MARKDOWN_PATH.name} từ "
            f"{PRODUCT_TEMPLATE_PATH.name}."
        )
