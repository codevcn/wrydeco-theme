...
The generated CSV uses the same 54-column structure as the supplied Shopify
product export.
"""
...
def load_settings(config_path: Path) -> Settings:
    ...
    tags = raw.get("tags", ["amazon-import"])
    ...
    return Settings(
        ...
        status=clean_text(raw.get("status", "draft")) or "draft",
...
def collect_image_records(data: dict[str, Any], title: str) -> list[dict[str, str]]:
    ...
    for item in sorted_gallery:
        # source_url is intentionally preferred because the prior watermark/upload
        # step replaces this field with the Shopify Content Files URL.
        url = clean_text(item.get("source_url") or item.get("resolved_url"))
...
def build_variant_options(
    ...
    for customization_index, customization in enumerate(
        ...
        for option_index, raw_option in enumerate(
            customization.get("options") or [], start=1
        ):
...
def build_tags(
    ...
    if asin:
        tags.append(f"amazon-asin-{asin.casefold()}")

    if settings.extra_tags_from_attributes:
        attributes = swatch.get("product_attributes") or data["product"].get(
            "product_attributes", {}
        )
        for key in ("material", "color", "finish_type", "shape"):
...
def resolve_product_type(data: dict[str, Any], settings: Settings) -> str:
    return settings.product_type or clean_text(data.get("product_type")) or "Home"
...
def build_product_rows(
    ...
    vendor = settings.vendor or get_attribute_value(
        attributes, "brand", "brand_name", "manufacturer"
    )
    material = get_attribute_value(attributes, "material", "product_material")
    wood_type = get_attribute_value(attributes, "wood_type")
            ...
            row.update(
                {
                    ...
                    "Body (HTML)": body_html,
                    "Vendor": vendor,
                    "Product Category": settings.product_category,
